<template>
    <CustomerLayout>
        ...
        <div class="feature-strip overflow-x-auto border-y border-gold-200 bg-cream-50 text-brand-800 scrollbar-hide"><div class="mx-auto flex w-max min-w-full max-w-7xl items-center justify-start gap-2 px-4 py-3 text-xs font-semibold tracking-wide sm:flex-wrap sm:justify-center sm:gap-x-8 sm:gap-y-1 sm:px-6 sm:text-sm lg:px-8"><span class="inline-flex shrink-0 items-center gap-2 rounded-full border border-sage-200 bg-white px-3 py-1.5 shadow-sm"><span class="h-1.5 w-1.5 rounded-full bg-sage-500"></span>Freshly baked daily</span><span class="inline-flex shrink-0 items-center gap-2 rounded-full border border-gold-200 bg-white px-3 py-1.5 shadow-sm"><span class="h-1.5 w-1.5 rounded-full bg-gold-500"></span>Handcrafted with care</span><span class="inline-flex shrink-0 items-center gap-2 rounded-full border border-brand-100 bg-white px-3 py-1.5 shadow-sm"><span class="h-1.5 w-1.5 rounded-full bg-brand-500"></span>Pickup & delivery</span><span class="inline-flex shrink-0 items-center gap-2 rounded-full border border-sage-200 bg-white px-3 py-1.5 shadow-sm"><span class="h-1.5 w-1.5 rounded-full bg-sage-500"></span>Happiness in every bite</span></div></div>
        ...
        <section v-if="allProducts.length" class="section-shell"><div class="mt-7 rounded-2xl border border-brand-100 bg-white/90 p-3 shadow-soft sm:flex sm:items-center sm:justify-between sm:gap-5"><div class="px-1"><p class="eyebrow">Shop by delivery option</p><p class="text-xs font-semibold text-brand-500">Find products that match how you want to receive them.</p></div><div class="mt-3 flex gap-2 overflow-x-auto scrollbar-hide pb-1 sm:mt-0"><button v-for="option in deliveryFilters" :key="`home-product-filter-${option.value}`" type="button" @click="productDeliveryFilter = option.value; showAllProducts = false" :class="productDeliveryFilter === option.value ? 'border-brand-700 bg-brand-700 text-white' : 'border-brand-200 bg-cream-50 text-brand-700'" class="whitespace-nowrap rounded-full border px-3 py-2 text-xs font-bold transition hover:border-brand-500">{{ option.label }}</button></div></div><div class="flex items-end justify-between gap-5"><div><p class="eyebrow">Fresh counter</p><h2 class="section-title text-left">All products</h2><p class="section-copy text-left">Explore the full selection available at your outlet.</p></div><Link :href="route('products.index')" class="text-sm font-bold text-brand-600 hover:text-brand-500">Shop all <span class="ml-2">→</span></Link></div><div v-if="filteredAllProducts.length" class="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-6 lg:grid-cols-4"><ProductCard v-for="product in filteredDisplayedAllProducts" :key="product.id" :product="product" @add="addToCart" /></div><div v-else class="mt-8 rounded-2xl border border-dashed border-brand-200 bg-brand-50/60 p-6 text-center text-sm text-brand-500">No products match this delivery option at your selected outlet.</div><div v-if="filteredAllProducts.length > 8" class="mt-8 flex justify-center"><button type="button" @click="showAllProducts = !showAllProducts" class="inline-flex items-center gap-2 rounded-full border border-brand-300 bg-white px-5 py-3 text-sm font-bold text-brand-700 shadow-soft transition hover:-translate-y-0.5 hover:border-brand-500 hover:bg-brand-50">{{ showAllProducts ? "Show fewer products" : "Show all products" }} <span>{{ showAllProducts ? "↑" : "↓" }}</span></button></div></section>
...
const productDeliveryFilter = ref('');
const deliveryModeFor = product => {
    const mode = product?.delivery_mode && product.delivery_mode !== 'inherit'
        ? product.delivery_mode
        : (product?.category?.delivery_mode || 'both');
    return mode === 'home_delivery' ? 'home_delivery' : mode === 'pickup' ? 'pickup' : 'both';
};
const filteredAllProducts = computed(() => allProducts.value.filter(product => {
    if (!productDeliveryFilter.value) return true;
    if (productDeliveryFilter.value === 'national') return product.allow_national_delivery === true;
    return deliveryModeFor(product) === productDeliveryFilter.value;
}));
const filteredDisplayedAllProducts = computed(() => showAllProducts.value ? filteredAllProducts.value : filteredAllProducts.value.slice(0, 8));
