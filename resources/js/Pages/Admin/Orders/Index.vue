            ...
            <table class="w-full">
                ...
                <tbody class="divide-y">
                    <tr v-for="order in orders.data" :key="order.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">{{ order.branch?.name }}</td>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">৳{{ order.total }}</td>
                        <td class="px-4 py-3"><span class="px-2 py-1 rounded-full text-xs capitalize" :class="statusClass(order.status)">{{ order.status.replace(/_/g, ' ') }}</span></td>
