...
const save = (product, amount = 1) => { let cart = []; try { cart = JSON.parse(localStorage.getItem('cart') || '[]'); } catch {} const item = cart.find(i => i.product_id === product.id); if (item) { item.quantity += amount; item.image = product.image || item.image; item.category_name = product.category?.name || item.category_name || ''; item.allow_pickup = product.allow_pickup !== false; item.allow_home_delivery = product.allow_home_delivery !== false; } else cart.push({ product_id: product.id, name: product.name, category_name: product.category?.name || '', image: product.image || '', price: Number(product.effective_price), quantity: amount, allow_pickup: product.allow_pickup !== false, allow_home_delivery: product.allow_home_delivery !== false }); localStorage.setItem('cart', JSON.stringify(cart)); window.dispatchEvent(new Event('cart-updated')); };
